...
from metrics import calculate_energy_pl, calculate_onevrest_acc
import pandas as pd
...
def single_trial(X, labels, label_rate, graph, alpha, lmda, max_iters=10000, tol=1e-8):
    """runs a single trial of training total variation poisson learning on the mnist dataset with all 10 digits 0-9.
    ...
    for positive_label in range(10): # iterate over digits 0-9 using a one-v-rest binary classifcation approach
        ...
        i = 0
        energy_list = [] # used to check energy every 100 iterations
        ...
        while i < max_iters:
            ...
            if i % 100 == 0 and i != 0: # print accuracy and energy at every 100th iteration
                ...
                if i > 1000: # lower alpha every 100 iterations after 1000 iterations
                    dynamic_alpha = dynamic_alpha / 5
...
if __name__ == "__main__":
    ...
    for trial_num in range(num_trials):
        ...
        for label_rate in range(1, 6): # we test algorithsm from label rates of 1 to 5 labels per class
            ...
            lr, a, l, acc = single_trial(X, labels, label_rate, graph, alpha, lmda)
