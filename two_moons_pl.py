...
for binary classification using the two moons dataset.
...
def single_trial(X, labels, label_rate, graph, alpha, lmda, max_iters=10000, tol=1e-8):
    """runs a single trial of training total variation poisson learning on the two moons dataset for binary classification.
    ...
    w_reweighted = graph.reweight(train_ind) # reweight graph based on reweighting scheme described in graphlearning package
    ...
    while i < max_iters:
        numerator = xi + (alpha * graph.gradient(np.sign(graph.divergence(xi, weighted=True) + ((lmda ** -1) * f)), weighted=False))
        denominator = alpha * np.absolute(graph.gradient(np.sign(graph.divergence(xi, weighted=True) + ((lmda ** -1) * f)), weighted=False))
...
if __name__ == '__main__':
    ...
    csv_output_path = f'./results/twomoons_pl_numtrials{num_trials}.csv'
